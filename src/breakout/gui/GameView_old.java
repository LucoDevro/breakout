...
public class GameView_old extends JPanel {
	 ...
	 */
	public GameView_old(BreakoutState breakoutState) {
	...
	private void paintBalls(Graphics g) {
		...
		for (Ball ball : breakoutState.getBalls()) {
			...
			Vector diameterRight = new Vector(ball.getDiameter(),0);
			Vector diameterUp = new Vector(0,ball.getDiameter());
			Point tl = ball.getCenter().minus(diameterRight).minus(diameterUp);
			Point br = ball.getCenter().plus(diameterRight).plus(diameterUp);
			paintBall(g, tl, br);
		}
	}
	...
	private void paintBlocks(Graphics g) {
		...
		for (BlockState block : breakoutState.getBlocks()) {
			// DONE: figure out top-left and bottom right Point of block
